...
from 
(select distinct patient_id,identifier,date_created from patient_identifier where identifier_type=3) as a join
(select distinct person_id,obs_datetime from obs where obs_datetime between cast('#startDate#' as date) and cast('#endDate#'  as date) and concept_id=2403 and voided=0) as b 
...
(select person_id,person_attribute_id,value from person_attribute where person_attribute_type_id=26) as pc on a.patient_id=pc.person_id left outer join
 ...
 (select person_id,person_attribute_id,value from person_attribute where person_attribute_type_id =28) as msc on a.patient_id= msc.person_id left outer join
 (select distinct patient_id,identifier,date_created from patient_identifier where identifier_type=5) as art_num on a.patient_id = art_num.patient_id;
